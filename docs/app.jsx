// NAGOMI PowerHAL - EAS (Eat Aware Scheduler)
// Static version for GitHub Pages

const { useState, useEffect, useRef } = React;
const { Camera, Coins, Calculator, RotateCcw, Settings, Zap, Brain, BookOpen, X, Link: LinkIcon, RefreshCw, Database, Filter, ArrowUpDown, TrendingUp, DollarSign, Utensils, Plus, Minus, Trash2, Edit2, Save, Check, LayoutDashboard, History: HistoryIcon, UtensilsCrossed, Droplets, Clock, ChevronRight, PenTool, Search, Timer, Cpu, Activity } = lucideReact;

// --- API Configuration ---
// Set this to your Vercel deployment URL, or leave empty to use local API key
const VERCEL_API_URL = ""; // e.g., "https://your-app.vercel.app/api/analyze"
...
const NagomiApp = () => {
  ...
  useEffect(() => {
    ...
    if (storedKey) setApiKey(storedKey); else if (!VERCEL_API_URL) setShowSettings(true);
    ...
    try {
      ...
      if (VERCEL_API_URL) {
        ...
        const response = await fetch(VERCEL_API_URL, {
          ...
          body: JSON.stringify({ image: base64Image, menuDB })
        });
      ...
      const enhancedItems = result.items.map(i => {
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-stone-900 border border-stone-700 p-6 rounded-2xl w-full max-w-sm space-y-4">
            ...
            <div className="space-y-3 text-sm">
              ...
              {VERCEL_API_URL && (
                <div className="p-2 bg-green-900/20 border border-green-800 rounded text-xs text-green-400 font-mono">API_MODE: SECURE_PROXY</div>
              )}
